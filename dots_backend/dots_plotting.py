...
def do_pcaplot(experiment, show=False, image=False, html_file='pcaplot.html'):
    ...
    source = bp.ColumnDataSource(data=dict(x=pca_df['xvals'].tolist(), y=pca_df['yvals'].tolist(), group=pca_df['group'].tolist(), sampleid=pca_df['sampleid'].tolist(), colour=pca_df['colour'].tolist()))
...
def do_volcanoplot(experiment, groups, show=False, image=False, html_file='volcano_plot.html'):
    ...
    source = bp.ColumnDataSource(data=dict(logfc=merged_df[column].tolist(), pval=merged_df['p_val_adj'].tolist(), gene=merged_df['GeneName'].tolist(), y=merged_df['neg_log_10_p_val'].tolist(), colour=merged_df['colour'].tolist()))
    ...
    volcanoplot.circle(x='logfc', y='y', color='colour', fill_alpha=0.4, line_alpha=0, size=5, source=source)
...
def do_clusters_plot(experiment, show=True, image=False, html_file='clustersplot.html'):
    ...
    grid = bl.gridplot(children=plots)
