...
class Experiment:
    ...
    def baseline_to_median(self):
        ...
        # self.df[norm_exp_cols] = self.df[norm_exp_cols].apply(lambda x : (x - x.median(axis=0)) if not len(x) % 2 == 0 else (x - sorted(x)[len(x)/2]), axis=1)
                ...
                # Replace the horrid lambda function with a more comprehensible loop. Start by getting the
                # median of each row
...
def read_experiment(array_filenames, baseline=False, annotations_file=None):
    ...
    ## NOTE: This assumes the file name has only one underscore.  We should probably 
